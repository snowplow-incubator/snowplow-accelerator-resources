...
def main():
    ...
    views_and_transactions = views_and_transactions.rename(columns = {'views' : 'Views', 'transactions': 'Transactions'})
    ...
    with col2:
        fig = px.line(product_views.rename(columns = {'product_views': 'Number of Views', 'date' : 'Date'}), x = 'Date', y = 'Number of Views')
        fig.update_layout(
            height=300,
            width=700,
            margin={"l": 20, "r": 20, "t": 25, "b": 0},
    ...
    checkout_funnel = checkout_funnel.rename(columns = {'session_entered_at_step' : 'Session Entered at Step', 'step' : 'Step', 'volume' : 'Volume'})
    ...
    sessions_funnel = sessions_funnel.rename(columns = {'session_entered_at_step' : 'Session Entered at Step', 'volume' : 'Volume', 'step' :'Step'})
